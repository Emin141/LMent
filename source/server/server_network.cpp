...
/* ------------------------------------------------------------------------------------------------------------------ */
void ServerNetwork::set_port(int port) {
    port_ = port;
}
/* ------------------------------------------------------------------------------------------------------------------ */
...
void ServerNetwork::listen_for_connections() {
    ...
    while (acceptedPlayersCount < playerCount_) {
        ...
        if (socketListener_.accept(*newSocket) == sf::Socket::Done) {
            ...
            spdlog::info("Accepted player from {}. Assigning player ID {}", newSocket->getRemoteAddress().toString(),
                         acceptedPlayerId);
        ...
        for (auto it = sockets_.begin(); it != sockets_.end();) {
            ...
            if (it->second->receive(dummyPacket) == sf::Socket::Disconnected) {
                spdlog::warn("Player {} has disconnected. Connection is not saved.", it->first);
    ...
    spdlog::info("Server has accepted sufficient number of players to start the match.");
...
void ServerNetwork::broadcast(MessageType messageType) {
    switch (messageType) {
        ...
        default:
            spdlog::warn("No implementation for broadcast of message type {}", static_cast<uint16_t>(messageType));
    }
}
/* ------------------------------------------------------------------------------------------------------------------ */
void ServerNetwork::broadcast_game_started() {
    for (const auto& [playerId, socket] : sockets_) {
        ...
        if (sockets_[playerId]->send(sendPacket) != sf::Socket::Done) {
            spdlog::error("Failed to send GameStarted message to player {} at {}.", playerId,
